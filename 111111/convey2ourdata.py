    ...
    with open(image_path, 'w') as file:
        for image in images:
            ...
            if  image_count[camera_id] <  20:
                ...
                original_image = Image.open(os.path.abspath(f"data/input/undis/{images[image].name}"))
                if(targetWidth == 1):
                    scale = 1
                else:
                    scale = cameras[camera].width/targetWidth
                ...
                region.save(f"data/images/{images[image].name}")
...
cameras,images = read_xml(getReadFilePath(os.path.abspath(f"data/input/CC"),"xml"))
save_colmap(cameras,images,os.path.abspath(f"data/sparse/0"))
# end modified @ zc
