...
main_path = os.path.abspath(f"data")
depth_path = os.path.join(main_path, "depth")

# 获取undis目录中的所有文件数量
num = len(os.listdir(os.path.abspath(f"data/input/undis")))
# 读取CC文件
cc = os.path.abspath(f"data/input/CC")
dom = xml.dom.minidom.parse(getReadFilePath(cc,"xml"))
# 读取着色点云文件      
color = os.path.abspath(f"data/input/color")
